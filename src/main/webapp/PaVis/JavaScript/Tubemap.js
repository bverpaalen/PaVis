...
// which starts the process of creating a tube map visualization
    function create(params) {
        ...
        //favorite ophalen nodes informatie
...
// favorite
    function createTubeMap() {
        ...
        for (let i = tracks.length - 1; i >= 0; i -= 1) {
            if(tempTracks[i] == null){
                continue
            }
            if (!tempTracks[i].hasOwnProperty('type')) { // TODO: maybe remove "haplo"-property?
                tempTracks[i].type = 'haplo';
            }

            if (tempTracks[i].hasOwnProperty('hidden')) {
                if (tempTracks[i].hidden === true) {
                    tracks.splice(i, 1);
                    continue
                }
            }
            if (tempTracks[i].hasOwnProperty('indexOfFirstBase')) {
                trackForRuler = tempTracks[i].name;
            }
        }
...
// align visualization to the top and left within svg and resize svg to correct size
    function alignSVG() {
        ...
        const zoom = d3.zoom().scaleExtent([0.1, 5]).on('zoom', null) /*() => {
            console.log(d3.event.translate)
            svg.attr('transform', `translate(`+d3.event.translate+`) scale(`+d3.event.scale+`)`);
        });*/
        svg = svg.call(zoom).on('dblclick.zoom', null).append('g').attr('id','PaVis').attr('width',svg.width).attr('heigth',svg.height);
        recG = svg.append('g').attr('id','allPaths')
...
// calculate the order-value of nodes contained in sequence which are to the left of the first node which already has an order-value
    function generateNodeOrderTrackBeginning(sequence) {
        ...
        console.log(nodes[Math.abs(sequence[anchorIndex])])
...
// generate global sequence of nodes from left to right, starting with first track and adding other tracks sequentially
    function generateNodeOrder() {
        ...
        for (let i = 1; i < tracksAndReads.length; i += 1) {
            if (DEBUG||true) console.log(`generating order for track ${i + 1}`);
...
// draws nodes by building svg-path for border and filling it with transparent white
    function drawNodes(dNodes) {
        ...
        //favorite creating node/circle
        svg.selectAll('.node')
            .data(dNodes)
            .enter()
            .filter(function(node){return node.gene !== null})
            .append('circle')
            .attr('class','gene')
            .attr('id', d => d.name)
            .attr('cx',function(node){return node.x})
            .attr('cy',function(node){return node.y + node.contentHeight/2})
            .attr('r',10)
            .attr('fill','red')
...
// draw seqence labels for nodes
    function drawLabels(dNodes) {
        if (config.nodeWidthOption === 0) {
            ...
            // svg.append('text')
                ...
                .append('text')
                .attr('x', d => d.x - 4)
                .attr('y', function(node){return node.y + node.contentHeight/2 + 4})
                .text(d => d.seq)
                .attr('class','sequence')
    ...
    function drawTrackRectangles(rectangles, type) {
        ...
        //favorite
        console.log(svg.select('g').selectAll('trackRectangles'))
        svg.selectAll('trackRectangles')
            ...
            .filter(function(rectangle){return (rectangle.xEnd - rectangle.xStart) > 15})
            .attr('large',true)
    ...
    function drawLegend() {
        ...
        for (let i = 0; i < tracks.length; i += 1) {
            if (tracks[i].type === 'haplo') {
                ...
                content += `<tr><td><div style="height:20px;width:40px;background:${generateTrackColor(tracks[i], 'exon')}"><span class="glyphicon glyphicon-stop" aria-hidden="true"></span></td>`;
                if (tracks[i].hasOwnProperty('name')) {
                    content += `<td>${tracks[i].name}</td>`;
                } else {
                    content += `<td>${tracks[i].id}</td>`;
                }
                content += `<td><input type="checkbox" class="checkBox" checked=true id="showTrack${i}"></td>`;
                listeners.push(i);
            }
        }
        content += '</table';
...
// (plus no inversions involved)
    function mergeNodes() {
        ...
        for (let i = 0; i < nodes.length; i += 1) {/* eslint no-param-reassign: "off" */
            /* eslint no-lonely-if: "off" */
            /* eslint no-prototype-builtins: "off" */
            /* eslint no-console: "off" */
...
// (plus no inversions involved)
            ...
            function mergeableWithPred(index, pred, succ) {
                ...
                //console.log(predecessorIndex)
                if(predecessorIndex == null) return false;
            ...
            function mergeableWithSucc(index, pred, succ) {
                ...
                if (pred[successorIndex] == null) return false;
